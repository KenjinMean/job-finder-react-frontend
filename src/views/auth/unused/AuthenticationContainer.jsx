...
export default function Login() {
    ...
    },
    {
      ...
      onError: ({ response }) => {
        if (response.request.status === 422) {
          ...
          setError("Network problems. Please try again later.");
        } else {
  ...
  const { isLoading: loginLoading, isError } = loginMutation;
  ...
  const handleProviderLogin = (provider) => {
    ...
    const apiUrl = `http://localhost:8000/api/auth/${provider}/get-authorization-url`;
    axios
      .get(apiUrl)
      .then((response) => {
  ...
  return (
    <body className="flex justify-center min-h-screen text-gray-900 bg-gray-50">
      <div className="flex items-center justify-center flex-1 m-0 bg-white shadow sm:max-w-xl sm:m-14 sm:rounded-lg">
        <div className="p-6 sm:p-12">
          ...
          <div className="flex flex-col items-center mt-12">
            ...
            >
              {authServiceError && (
                <span className="text-red-500 blockmt-5">
                  {authServiceError}
                </span>
              )}
            </div>
